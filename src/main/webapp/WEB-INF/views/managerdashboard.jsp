...
<path
				...
				<div class="dropdown">
					...
					<ul class="dropdown-menu dropdown-menu-end"
						...
						<li class="dropdown-item">Hi ${sessionScope.user.tenantFname}</li>
	...
	<section>
		<div class="row">
			<div class="col-4 hero p-5" style="height: 100vh;">
				<div class="w-100">
					<a
						class="rounded fw-semibold d-block text-decoration-none px-5 my-2 py-2 text-dark ${param.section == 'hostels' ? 'active' : ''}"
						href="${pageContext.request.contextPath}/manager/dashboard?section=hostels"><i class="fa-solid fa-hotel pe-4"></i>Hostels</a> <a
						class="rounded fw-semibold d-block text-decoration-none px-5 my-2 py-2 text-dark ${param.section == 'profile' ? 'active' : ''}"
						href="${pageContext.request.contextPath}/manager/dashboard?section=profile"><i class="fa-solid fa-person-booth pe-4"></i>Rooms</a> <a
						class="rounded fw-semibold d-block text-decoration-none px-5 my-2 py-2 text-dark ${param.section == 'bookings' ? 'active' : ''}"
						href="${pageContext.request.contextPath}/manager/dashboard?section=bookings"><i class="fa-solid fa-shuffle pe-4"></i>Misc.</a>
				</div>


			</div>
			<div class="col-8 p-5 d-flex flex-column align-items-center">
				<c:choose>
					<c:when test="${param.section == 'hostels'}">
								<h1 class="mb-5">Add/ Edit Hostel</h1>
						<div class="d-flex justify-content-center" style="gap: 20px;">
							<div class="card w-50">
								<div class="card-header fw-bold">Add Hostel</div>
								<div class="card-body">
									<p class="card-text">Go through this to add a<br/>hostel.</p>
									<a
										href="${pageContext.request.contextPath}/hostel/showHostelForm"
										class="button w-100 btn-block mb-3 text-decoration-none text-dark fw-semibold">Add Hostel</a>
								</div>
							</div>
							<div class="card w-50">
								<div class="card-header fw-bold">Edit Hostel Details</div>
								<div class="card-body">
									<p class="card-text">Go through this to edit hostel details.</p>
									<a href="${pageContext.request.contextPath}/hostel/editHostel" class="button w-100 btn-block mb-3 text-decoration-none text-dark fw-semibold">Edit Hostel</a>
								</div>
							</div>
						</div>
					</c:when>
					<c:when test="${param.section == 'profile'}">
						<h1 class="mb-5">Add/ Edit Rooms</h1>
						<div class="d-flex justify-content-center" style="gap: 20px;">
							<div class="card w-50">
								<div class="card-header fw-bold">Add Room</div>
								<div class="card-body">
									<p class="card-text">Go through this to add a room<br/>to a hostel.</p>
									<a
										href="${pageContext.request.contextPath}/room/showroomform"
										class="button w-100 btn-block mb-3 text-decoration-none text-dark fw-semibold">Add Room</a>
								</div>
							</div>
							<div class="card w-50">
								<div class="card-header fw-bold">Edit Room Details</div>
								<div class="card-body">
									<p class="card-text">Go through this to edit room details.</p>
									<a href="${pageContext.request.contextPath}/room/editroomform" class="button w-100 btn-block mb-3 text-decoration-none text-dark fw-semibold">Edit Room</a>
								</div>
							</div>
						</div>
					</c:when>
					<c:when test="${param.section == 'bookings'}">
						<h1 class="mb-5">Miscellaneous</h1>
						<div class="d-flex justify-content-center" style="gap: 20px;">
							<div class="card w-50">
								<div class="card-header fw-bold">View all rooms</div>
								<div class="card-body">
									<p class="card-text">Go through this to view all<br/>rooms.</p>
									<a
										href="${pageContext.request.contextPath}/room/listrooms"
										class="button w-100 btn-block mb-3 text-decoration-none text-dark fw-semibold">Show all rooms</a>
								</div>
							</div>
							<div class="card w-50">
								<div class="card-header fw-bold">View feedback</div>
								<div class="card-body">
									<p class="card-text">Go through this to view all feedbacks.</p>
									<a href="${pageContext.request.contextPath}/feedback/viewfeedback" class="button w-100 btn-block mb-3 text-decoration-none text-dark fw-semibold">View feedback</a>
								</div>
							</div>
						</div>
					</c:when>
					...
					<c:otherwise>
						<h1 class="text-blue mb-4">Welcome ${sessionScope.user.tenantFname}</h1>
						<div class="w-75">
						<canvas id="revenueChart" width="400" height="200"></canvas>
						<p class="text-center text-grey" style="font-size: 14px;" id="revenueText">Revenue</p> 
						</div>
					</c:otherwise>
				</c:choose>
			</div>
...
<path d="M29.1089 40.401H2.88989V41.5512H29.1089V40.401Z" fill="#222831" />
	...
	<script
	    ...
	    document.addEventListener("DOMContentLoaded", function() {
            ...
            document.getElementById('revenueText').textContent = "Revenue generated in the year "+currentYear;
	    ...
	    // Convert the monthly revenue data into JavaScript arrays
        const labels = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];
        const revenueData = [];

        <c:forEach var="monthRevenue" items="${monthlyRevenue}">
            revenueData[${monthRevenue.month - 1}] = ${monthRevenue.revenue};  // Use month-1 because arrays are 0-indexed
        </c:forEach>

        // Fill missing months with 0
        for (let i = 0; i < 12; i++) {
            if (typeof revenueData[i] === 'undefined') {
                revenueData[i] = 0;
            }
        }
        ...
        const chart = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    data: revenueData,
                    backgroundColor: 'rgba(105,164,219, 0.2)',
                    borderColor: 'rgba(105,164,219, 1)',
                    borderWidth: 1
                }]
            },
            options: {
