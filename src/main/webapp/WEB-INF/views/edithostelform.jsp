...
<path
    ...
    <div class="row">
        ...
        <div class="col-8 p-5 d-flex flex-column justify-content-center">
            ...
            <div class="row justify-content-center">
                <div class="col-md-6">
                    <!-- Dropdown to select which hostel to edit -->
                    <form id="selectHostelForm">
                        <div class="mb-3 form-group form-floating">
                            <select id="hostelId" name="hostelId" class="form-select" onchange="this.form.submit()" required>
                                ...
                                <c:forEach var="hostel" items="${hostels}">
                                    <option value="${hostel.hostelId}" ${hostel.hostelId == (param.hostelId) ? 'selected' : ''}>
                                        ${hostel.hostelName}
                                    </option>
                                </c:forEach>
                            </select>
                            <label for="hostelId" class="form-label">Select Hostel to Edit:</label>
                            <div class="invalid-feedback">Please select a hostel.</div>
                        </div>
                    </form>

                    <!-- Form to edit the selected hostel -->
                    <form id="editHostelForm" action="${pageContext.request.contextPath}/hostel/updateHostel" method="post" novalidate>
                        <c:if test="${not empty selectedHostel}">
                            <div class="mb-3 form-group form-floating">
                                <input type="text" id="hostelName" name="hostelName" class="form-control" placeholder=""
                                       value="${selectedHostel.hostelName}" required minlength="3" maxlength="100">
                                <label for="hostelName" class="form-label">Hostel Name:</label>
                                <div class="invalid-feedback">Hostel Name is required.</div>
                            </div>
                            <div class="mb-3 form-group form-floating">
                                <input type="text" id="hostelLocation" name="hostelLocation" class="form-control" placeholder=""
                                       value="${selectedHostel.hostelLocation}" required minlength="3" maxlength="100">
                                <label for="hostelLocation" class="form-label">Location:</label>
                                <div class="invalid-feedback">Location is required.</div>
                            </div>
                            <div class="mb-3 form-group form-floating">
                                <textarea id="hostelDescription" name="hostelDescription" class="form-control" placeholder="" required minlength="10" maxlength="250">${selectedHostel.hostelDescription}</textarea>
                                <label for="hostelDescription" class="form-label">Description:</label>
                                <div class="invalid-feedback">Description is required.</div>
                            </div>
                            <div class="mb-3 form-group form-floating">
                                <input type="text" id="hostelAmenities" name="hostelAmenities" class="form-control" placeholder="" 
                                       value="${selectedHostel.hostelAmenities}" required minlength="3" maxlength="150">
                                <label for="hostelAmenities" class="form-label">Amenities:</label>
                                <div class="invalid-feedback">Amenities is required.</div>
                            </div>
                            <input type="hidden" name="hostelId" value="${selectedHostel.hostelId}">
                            <button type="submit" class="button w-100 btn-block mb-3">Update Hostel</button>
                        </c:if>
                    </form>
                </div>
            </div>
    ...
    (function() {
        'use strict';
        var form = document.getElementById('editHostelForm');
