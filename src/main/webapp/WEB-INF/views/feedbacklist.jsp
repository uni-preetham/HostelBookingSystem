...
<path
	...
	<section>
		<div class="custom-container hero rounded my-5">
						...
						<c:otherwise>
							<c:forEach var="feedback" items="${feedbacks}">
								<tr>
									...
									<td>${feedback.tenant.tenantFname}
										${feedback.tenant.tenantLname}</td>
									<td>${feedback.feedbackDescription}</td>
									<%-- <td>${feedback.feedbackStatus}</td>
									<td>
								<form class="d-flex" style="gap:10px;"
									action="${pageContext.request.contextPath}/feedback/update-status/${feedback.feedbackId}"
									method="post">
									<select name="status" class="form-select w-50">
										<option value="Pending"
											<c:if test="${feedback.feedbackStatus == 'Pending'}">selected</c:if>>Pending</option>
										<option value="Resolved"
											<c:if test="${feedback.feedbackStatus == 'Resolved'}">selected</c:if>>Resolved</option>
									</select>
