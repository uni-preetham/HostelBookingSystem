...
<path
    ...
    <div class="row">
        ...
        <img style="max-height: 600px; position: absolute; left:0px; bottom:0px;" src="https://acko-home-prod.ackoassets.com/next_assets/85687f46693cde070cad14ed9e765f691c7ebf85/_next/static/images/Sara-853a3edabf17a62181187aca0bbbc008.png">
        </div>
        ...
        <div class="col-8 p-5 d-flex flex-column">
            ...
            <div class="row justify-content-center">
                <div class="col-md-6">
                    <!-- Form to select hostel -->
                    <form id="selectHostelForm" action="${pageContext.request.contextPath}/room/editroomform" method="get" novalidate>
                        <div class="mb-3 form-group form-floating">
                            <select id="hostelId" name="hostelId" class="form-select" onchange="this.form.submit()" required>
                                <option value="" selected disabled>Select Hostel</option>
                                <c:forEach var="hostel" items="${hostels}">
                                    <option value="${hostel.hostelId}" ${hostel.hostelId == selectedHostelId ? 'selected' : ''}>
                                        ${hostel.hostelName}
                                    </option>
                                </c:forEach>
                            </select>
                            <label for="hostelId" class="form-label">Select Hostel</label>
                            <div class="invalid-feedback">Please select a hostel.</div>
                        </div>
                    </form>

                    <!-- Form to select room if a hostel is selected -->
                    <%-- <c:if test="${not empty selectedHostelId}"> --%>
                        <form id="selectRoomForm" action="${pageContext.request.contextPath}/room/editroomform" method="get" novalidate>
                            <input type="hidden" name="hostelId" value="${selectedHostelId}">
                            <div class="mb-3 form-group form-floating">
                                <select id="room" name="roomId" class="form-select" onchange="this.form.submit()" required>
                                    <option value="" selected disabled>Select Room</option>
                                    <c:forEach var="room" items="${rooms}">
                                        <option value="${room.roomId}" ${room.roomId == selectedRoom.roomId ? 'selected' : ''}>
                                            ${room.roomNumber}
                                        </option>
                                    </c:forEach>
                                </select>
                   ...
                   <%--  </c:if> --%>
                    ...
                    <!-- Form to edit room details if a room is selected -->
                   <%--  <c:if test="${not empty selectedRoom}"> --%>
                        <form id="editRoomForm" action="${pageContext.request.contextPath}/room/updateroom" method="post" novalidate>
                            <input type="hidden" name="roomId" value="${selectedRoom.roomId}">
                            
                            <!-- Room Number -->
                            <div class="mb-3 form-group form-floating">
                                <input type="text" id="roomNumber" name="roomNumber" class="form-control" placeholder=""
                                       value="${selectedRoom.roomNumber}" required minlength="1" maxlength="50">
                            ...
                            <!-- Room Floor Number -->
                            <div class="mb-3 form-group form-floating">
                                <input type="number" id="roomFloorNumber" name="roomFloorNumber" class="form-control"
                                       value="${selectedRoom.roomFloorNumber}" placeholder="" required min="1" max="100">
                                <label for="roomFloorNumber" class="form-label">Floor Number</label>
                                <div class="invalid-feedback">Floor Number is required.</div>
                            </div>

                            <!-- Room Type -->
                            <div class="mb-3 form-group form-floating">
                                <input type="text" id="roomType" name="roomType" class="form-control" placeholder=""
                                       value="${selectedRoom.roomType}" required minlength="1" maxlength="50">
                            ...
                            <div class="mb-3 form-group form-floating">
                                <input type="number" id="price" name="price" class="form-control" placeholder=""
                                       value="${selectedRoom.price}" required min="0" step="0.01">
                                <label for="price" class="form-label">Price</label>
                                <div class="invalid-feedback">Price is required and should be a positive number.</div>
                            </div>

                            <!-- Availability -->
                            <div class="btn-group mb-2" role="group" aria-label="Basic checkbox toggle button group">
                                <input type="checkbox" class="btn-check" id="availability" name="availability" ${selectedRoom.availability ? 'checked' : ''}>
                                <label class="btn btn-outline-primary" for="availability" id="availabilityLabel">Available</label>
                            </div>

                            <!-- Submit Button -->
                            <button type="submit" class="button w-100 btn-block">Update Room</button>
                        </form>
                   <%--  </c:if> --%>
                </div>
            </div>
